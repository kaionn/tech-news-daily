...
prompt には push 用 PAT が埋め込まれているため、`RemoteTrigger update` は auto-mode classifier（credential 実体化ガード）に必ずブロックされる。prompt 更新は claude.ai の routine 編集画面（https://claude.ai/code/triggers/trig_01LvUYkX4UXHkv8KDLsFH9eL）からユーザーが手動で貼り替える。セッション側は新 prompt を PAT プレースホルダ付きファイルで用意して渡すところまで。
