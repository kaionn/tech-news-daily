...
Your job: collect today's tech news via WebSearch, generate a rich HTML digest, update the Atom feed, archive yesterday's issue, self-review, then commit. Deployment to main + GitHub Pages is handled automatically by GitHub Actions — you only commit to your working branch.
...
## Step 8: Commit (NO push to main)

Commit the generated files to the current working branch. Do NOT change `git config user.name` / `user.email` — keep the identity the CCR harness has already configured (`Claude <noreply@anthropic.com>`). Changing the commit author breaks the harness's automatic branch push.

```bash
git add index.html archive/ feed.xml
git commit -m "$(date -u '+%Y-%m-%d') のテックニュースダイジェスト"
```

The commit message MUST stay in the exact form `YYYY-MM-DD のテックニュースダイジェスト` — the GitHub Actions auto-merge workflow matches this pattern to decide what to promote to main.

After you commit, your job is done. The CCR harness pushes this working branch to origin automatically, and `.github/workflows/auto-merge-digest.yml` fast-forwards the commit into `main` and deploys GitHub Pages. You do not run `git push`, `git remote set-url`, or handle any token.

## Important

- Do NOT run `git push`, `git remote set-url`, or reference any PAT/token. There is no secret in this routine anymore; deployment is handled by GitHub Actions after your branch is auto-pushed.
- Do NOT run `git config` or `git commit --amend`. Commit once with the harness's default identity.
...
- Keep the commit message format exactly `YYYY-MM-DD のテックニュースダイジェスト` (the auto-merge workflow depends on it).
