...
Your job: collect today's tech news via WebSearch, generate a rich HTML digest, update the Atom feed, archive yesterday's issue, self-review, then commit and push to main.
...
git remote set-url origin https://x-access-token:【ここに 1Password「GitHub PAT - tech-news-daily」の credential を貼る】@github.com/kaionn/tech-news-daily.git
git push origin main
```

## Important

- Do NOT echo or print the PAT token to stdout
- If WebSearch returns no results for a category, skip it rather than making up content
- If git push fails, report the error but do not retry with different auth methods
