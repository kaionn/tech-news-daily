...
The toc's last link (`<a href="trends.html">📈 Trends</a>`) must always be present: it points to the weekly plugin trends page generated by a separate workflow. It is a static link, not an anchor — no existence check needed, and it never changes day to day.
...
3. STRUCTURE: all 5 sections present with correct ids and emoji titles; stats-bar / toc (including the trailing `trends.html` link) / numbers-bar present; first Top Story has `deep-dive` class + `editorial`; only CSS classes defined above are used.
